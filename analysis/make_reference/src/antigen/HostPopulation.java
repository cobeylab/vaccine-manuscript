...
//		System.err.println(sim.getDay()+" "+p+" is at: "+getFrequency(p, currentPhenotypes));
	...
	public void resetStrains(SqlJetDb readDb){
		...
		if(nInfections > 0){
			...
			try{
				...
				do{
					Phenotype p = new Phenotype((double) c.getFloat("ag1"), (double) c.getFloat("ag2"));
					int niStrain = (int) Math.round(c.getFloat("freq") * nInfections);
...
//				}
	...
	public void printSomeHosts(PrintStream stream){
		...
		for (int index : sampleIndices) {
			Host h = susceptibles.get(index);
			stream.printf("%.4f\t%d\t%d\t%f\t%f\t%f\t%d\t%d\n",
					sim.getDate(), 
