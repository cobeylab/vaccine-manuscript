...
//	}
	...
	public void printPopulationSummary(PrintStream stream){
		//for counting true susceptibles and recovereds
		// number to sample
		int sampleS = (int) (getS() * params.printSampleRateS);
		int sampleI = (int) (getI() * params.printSampleRateI);
		int sampleR = (int) (getT() * params.printSampleRateR);
		// sampled counts
		int nSusceptible = 0;
		int nInfected = 0;
		int nRecovered = 0;
		...
		Set<Integer> sampleIndices = chooseRandomIndices(susceptibles.size(), sampleS);
		
		// step through susceptibles and print
		for (int index : sampleIndices) {
			Host h = susceptibles.get(index);
			if(h.getVaccinationHistory(sim) != null && h.getVaccinationHistory(sim).size() > 0){
				nVaccinated = nVaccinated + 1;
			}
			else{
				if(h.getHistoryLength() > 0){
		...
		stream.print(sim.getDate() + "," + nSusceptible + "," + nRecovered + "," + nInfected + "," + nVaccinated + "\n");		
	}
	
	public void printHostPopulation(PrintStream stream) {
		int nSusceptibles = (int) (getS() * params.printSampleRateS);
		int nInfecteds = (int) (getI() * params.printSampleRateI);
		int nRecovereds = (int) (getT() * params.printSampleRateR);
		
		// step through susceptibles and print
		//stream.print("Susceptibles\n");
		for (int i = 0; i < nSusceptibles; i++) {
			Host h = getRandomHostS();
			//stream.print(deme + ":");
			stream.print(sim.getDate());
			stream.print(":S:");
			h.printInfection(stream);
			stream.print(":");
