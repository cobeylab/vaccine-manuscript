...
public class Simulation {
	...
	private List<Phenotype> currentReferenceStrains;
	...
	private PrintStream hostOutStream;
	private PrintStream phenotypeOutStream;
	
	private SqlJetDb sampleDb;
	private SqlJetDb referenceDb;
	private SqlJetDb readDb;
	...
	public Simulation(Parameters params) {
		...
		currentReferenceStrains = new ArrayList<Phenotype>();
...
//			try{
//				BufferedReader br = new BufferedReader(new FileReader("refPhenotypes.tsv"));
//				br.readLine();
//				while ((line = br.readLine()) != null) {
//
//					String[] row = line.split(splitBy);
//					Phenotype p = new Phenotype(Double.parseDouble(row[1]), 
//							Double.parseDouble(row[2]),
//							Double.parseDouble(row[3]), 
//							Double.parseDouble(row[0]));
//					currentReferenceStrains.add(p);
//				} 
...
//		}
	...
	public HostPopulation getDeme(int i){
		return(demes.get(i));
	}
...
//		}
	...
	public double getAverageRisk(Phenotype p) {
		...
		for (int i = 0; i < 10000; i++) {
			...
			double lastVaccineDate = h.getLastVaccineDate();
			double date = getDate();
			averageRisk += p.riskOfInfection(immuneHistory, vaccinationHistory, lastVaccineDate, date,
	...
	public void printLongitudinal(){
		...
		try{
			...
			for (int i = 0; i < params.demeCount; i++) {
				HostPopulation hp = demes.get(i);
				hp.printLongitudinal(longStream);
			}
			longStream.close();
		} catch(IOException ex) {
			System.out.println("Could not write to file"); 
	...
	public void printViruses() {
		...
		try {
			File virusFile = new File("out.viruses");
			virusFile.delete();
			virusFile.createNewFile();
			PrintStream virusStream = new PrintStream(virusFile);
			for (int i = 0; i < params.demeCount; i++) {
				HostPopulation hp = demes.get(i);
				hp.printViruses(virusStream);
			}
			virusStream.close();
		} catch(IOException ex) {
			System.out.println("Could not write to file"); 
	...
	public void stepForward() {
		...
		for (int i = 0; i < params.demeCount; i++) {		
			...
			if(params.useReferenceStrains){
				if(!hp.resetStrains(readDb)){ // returns true if reference is not extinct
					System.err.println("I AM EXTINCT");
					for(Host h : hp.infecteds){
						hp.susceptibles.add(h);
					}
					hp.infecteds.clear();
//					try {
...
//					}
			...
			if(params.writeReference){
				hp.writePhenotypesToSqlite(referenceDb, phenotypeOutStream);
			}
	...
	public void run() {
		...
		try {
			...
			hostOutStream.println("date,S,R,I,V");

			//hostOutStream.println("date:class:infection:history");
			...
			while (timestep <= endstep) {
				...
				if (day % (double) params.printStep < params.deltaT) {			
					...
					if(params.printHosts && day > 0){
						for (int i = 0; i < params.demeCount; i++) {		
							HostPopulation hp = demes.get(i);
							//hp.printHostPopulation(hostOutStream);
							hp.printPopulationSummary(hostOutStream);
						}
					}
...
//			
	...
	public void initReferenceDb() {
		try{
			File outputDbFile = new File("reference.sqlite");
			outputDbFile.delete();
			referenceDb = new SqlJetDb(outputDbFile, true);
			referenceDb.open();

			referenceDb.beginTransaction(SqlJetTransactionMode.WRITE);
			referenceDb.createTable("CREATE TABLE phenotypes (day REAL, ag1 REAL, ag2 REAL, freq REAL);");
			referenceDb.commit();
		}
		catch(SqlJetException e) {
			throw new RuntimeException(e);
	...
	public void initReadDb() {
		try{
			File readDbFile = new File(params.referenceFile);
			readDb = new SqlJetDb(readDbFile, true);
			readDb.open();
